  ...
  <v-container class="py-6">
    ...
    <v-card elevation="8" class="rounded-xl">
      ...
      >
        ...
        <!-- Fecha pago visible: fecha_pago -> fechaPago -> confirmado_(at) -> created_(at) -->
        <template #item.fecha_pago="{ item }">
          <span v-if="firstFecha(item)">{{ asDate(firstFecha(item)) }}</span>
          <span v-else>—</span>
        </template>

        <!-- Total = total_factura (fallback total) -->
    ...
    <!-- DIALOG DETALLE (layout idéntico al modal de Confirmar) -->
    <v-dialog v-model="dialog.open" max-width="780">
      <v-card>
        ...
        <v-card-text>
          ...
          <v-row>
            ...
            <!-- Col derecha: Turno asociado (idéntico) -->
            <v-col cols="12" md="6">
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn variant="text" @click="dialog.open=false">Cerrar</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
async function openDetailById(id: number | string) {
  try {
    const full = await FacturacionService.getById(id) // DTO camelCase desde /show
...
function estadoTurnoColor(s?: string | null) {
  const v = String(s || '').toLowerCase()
  if (v.includes('activo')) return 'success'
  if (v.includes('final')) return 'blue-grey'
...
function fileHref(t?: any | null) {
  const p = t?.filePath || t?.file_path
  if (!p) return ''
  return String(p).startsWith('/') ? String(p) : `/${String(p)}`
}

/* ==== Helpers para evitar mezclar ?? y || en el template ==== */
function pickTotal(it: any) {
  // Prioridad: totalFactura (camel) -> total_factura (snake) -> total -> 0
...
/* ------- AJUSTADOS: soportan snapshots snake_case y relaciones ------- */
...
function getServicioCodigo(it: any) {
  return it?.servicio_codigo
      ?? it?.servicioCodigo
      ?? it?.servicio?.codigoServicio
      ?? '—'
}
...
.thumb-wrap { display: inline-block; border: 1px solid #eee; background: #fafafa; border-radius: 8px; padding: 6px; }
.thumb-wrap img { max-width: 100%; max-height: 260px; display: block; border-radius: 6px; }
</style>
